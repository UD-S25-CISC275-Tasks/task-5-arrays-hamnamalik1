...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    return messages
        .filter((message) => !message.endsWith("?")) // Remove messages ending with "?"
        .map((message) =>
            message.endsWith("!") ? message.toUpperCase() : message,
        ); // Uppercase messages ending with "!"
};
...
export function injectPositive(values: number[]): number[] {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        if (values[i] < 0) {
            result.push(sum - values[i]); // Exclude the negative number itself from sum
            return result.concat(values.slice(i + 1)); // Append the rest of the array
        }
    }
